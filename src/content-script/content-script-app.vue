  ...
  methods: {
    init_step_extract: function( config ) {
      ...
      chrome.storage.local.get(null).then(hotpotato => {
        ...
        vue.$nextTick(function() {
          ...
          hotpotato = hotpotato || {};
          if ( Array.isArray(hotpotato.books) ) config.oldBooksLength = hotpotato.books.length;
    ...
    outputCleanup( hotpotato ) {
      ...
      let removeStragglers = function( key ) {
        if ( Array.isArray(hotpotato[ key ]) ) {
          _.each(hotpotato[ key ], function( book ) {
            ...
            if ( key === 'books' && Array.isArray(book.series) ) {
              _.each( book.series, ( series ) => {
                const foundSeries = _.find(hotpotato.series, { asin: series.asin});
                if (!foundSeries) return;
                if (!Array.isArray(foundSeries.books)) foundSeries.books = [];
                if (!Array.isArray(foundSeries.allBooks)) foundSeries.allBooks = [];
      ...
      if (Array.isArray(hotpotato.series)) {
        _.each(hotpotato.series, series => {
          if (Array.isArray(series.allBooks)) {
      ...
      if (
          Array.isArray(hotpotato.books) && hotpotato.books.length > 0 &&
          Array.isArray(hotpotato.wishlist) && hotpotato.wishlist.length > 0
      ) {
    ...
    saveExtractionSoFar( hotpotato, callback ) {
      ...
      if (!hotpotato.chunks ) {
        if ( Array.isArray(hotpotato.books) ) this.addedOrder(hotpotato.books);
        if ( Array.isArray(hotpotato.books) ) this.languageCorrections(hotpotato.books);
        if ( Array.isArray(hotpotato.wishlist) ) this.addedOrder(hotpotato.wishlist);
